...
- **Schema**: Defined in `db/schema.sql` (executed via wrangler).
    ...
    - `AnalyticsEvents`: Stores raw pageview/event data (Privacy-friendly).
...
- **Forms**: We use [Astro Actions](https://docs.astro.build/en/guides/actions/) for all form handling (`src/actions/`).
    - **API Routes**: Only used for high-performance/external endpoints like Analytics Tracking (`src/pages/api/analytics/track.ts`).
